...
export const loadMWOs = async () => {
  // const res = await httpParams(process.env.API.MWO);
  const res = await httpParams("http://localhost:3000/api/mwo");
  const { data: mwoList } = await res.json();
  console.log('mwoList', mwoList[0]);
  return mwoApollo(mwoList)
...
export const deleteMWOHandler = async (deleteMWOid) => {
  ...
  // delete - DB
  const res = await httpParams(process.env.API.MWO, 'DELETE', { mwoUUID })
  await res.json()
...
export const addMWOHandler = async (formData) => {
  let mwoState = [...mwoApollo()];
  // Check Duplicate
  const res = await httpParams(process.env.API.MWO, "POST", { mwoData: formData })
...
export const updateMWOHandler = async (formData) => {
  ...
  // Database
  const res = await httpParams(process.env.API.MWO, "PATCH", {
    mwoUUID: targetUUID,
    mwoData: formData
  })
