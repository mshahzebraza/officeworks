...
export const addTxnHandler = async (txnData, multiple = false) => {
  ...
  if (multiple) {
    ...
    const uniqueTxnList = txnData.filter(
      txn => {
        return transactionState.findIndex(transaction => transaction.tid === txn.tid) < 0
      })
    // 2. Database
    ...
    const isUnique = transactionState.findIndex(txn => txn.tid === txnData.tid) < 0;
...
export function mapPOtoTransaction(data = {}, poItemIdx = Math.round(Math.random() * 9), totalPoItems = 999, refType, refId, supplier) {
  ...
  return {
    ...
    */
    ...
    tid: `${refType}#${refId}_${poItemIdx + 1}/${totalPoItems}_${epochTimeOfTxn}`, // auto-generated transaction id
