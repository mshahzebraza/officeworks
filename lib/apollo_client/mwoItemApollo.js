...
export const deleteMWOitemHandler = async ([activePOid, itemDeleteId]) => {
     ...
     const activeSourceIndex = mwoStateList.findIndex(po => po.refId === activePOid)
     ...
     console.log('activeSource: ', deepClone(activeSource));
     ...
     const { success, data, error, message } = await request(
          {
               ...
               method: 'DELETE',
               params: {
                    mwoUUID: activeSourceUUID, // po.refId
     ...
     { // Update the module state (Delete or unlink)
          ...
          const moduleTargetIsExclusive = activeModule.linkedPOs.length === 1;
...
export const addMWOitemHandler = async ([activePOid, itemData]) => {
     // TODO: update the fetch property of the states
...
export const updateMWOitemHandler = async ([activePOid, itemData]) => {
