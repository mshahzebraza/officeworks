...
export const deleteProjHandler = async (projectNomenclature) => { // received project nomenclature
  ...
  if (targetExists) {
    ...
    // 1. Database Delete
    await httpParams(
      `${process.env.API.PROJECT}?projectUUID=${targetUUID}`,
      'DELETE'
    )
...
export const deleteProjModHandler = async ([pjCatName, pjId, partId]) => {
  ...
  if (targetExists) {
    ...
    // 1. Database Delete
    const res = await httpParams(
      `${process.env.API.PROJECT_PART}?projectUUID=${targetParentUUID}&partID=${targetID}`,
      "DELETE"
    )
    // 2. Local State Delete
...
export const updateProjModHandler = async ([pjCatName, pjId, formData]) => {
  ...
  if (targetExists) { // if module is found 
    ...
    // 2. Local state Update
    targetParent.parts.splice(targetIndex, 1, updatedPart)
...
export const addProjModHandler = async ([pjCatName, pjId, formData]) => {
  ...
  if (isUnique) {
    ...
    // 1. Database Create
    const res = await httpParams(
      `${process.env.API.PROJECT_PART}?projectUUID=${targetParentUUID}`,
      "POST",
      {
        partData: formData
      }
...
export const deleteProjAssyHandler = async ([pjCatName, pjId, removeAssemblyId]) => { // action.payload = [formData, =>ldItems]
  ...
  if (targetExists) {
    ...
    // 1. Database Delete
    await httpParams(
      `${process.env.API.PROJECT_ASSY}?projectUUID=${targetParentUUID}&assemblyID=${targetID}`,
      "DELETE"
    )
    // 2. Local State Delete
...
export const addProjAssyHandler = async ([pjCatName, pjId, formData]) => { // action.payload = [formData, =>ldItems]
  ...
  if (isUnique) {
    ...
    // 1. Database Create
    const res = await httpParams(
      `${process.env.API.PROJECT_ASSY}?projectUUID=${targetParentUUID}`,
      "POST",
      {
