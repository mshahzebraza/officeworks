...
import moduleApollo from './poItemApollo';

// Target: PO/POItem
// ! All of the following code is written for PO and must be changed to be used for MWO
export const deleteMWOitemHandler = async ([activeSourceID, itemDeleteId]) => {
     ...
     console.log('itemDeleteId: ', itemDeleteId);
     ...
     console.log('activeSourceUUID', activeSourceUUID);
     console.log('activeModuleUUID', activeModuleUUID);
     ...
     { // Update the module state (Delete or unlink)
          ...
          if (!activeModule.linkedMWOs.includes(moduleSource._id)) {
               console.error('Module target is not linked to current PO. Returning null');
               return null;
          }

          const moduleTargetIsExclusive = activeModule.linkedMWOs.length === 1;

          if (moduleTargetIsExclusive) {
...
export const addMWOitemHandler = async ([activeSourceID, itemData]) => {
...
export const updateMWOitemHandler = async ([activeSourceID, itemData]) => {
     ...
     const activeSourceIndex = mwoStateList.findIndex(po => po.refId === activeSourceID);
...
export const updateMWOitemSpecHandler = async ([specFormData]) => {
