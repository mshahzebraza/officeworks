...
import { checkDataType, httpParams, request } from '../../helpers/reusable';
...
export const addPOHandler = async (newPOvalues) => {
  ...
  if (isUnique) {
    // Database
    const { data: createdPO } = await request(
      {
        url: process.env.API.PO,
        method: 'POST',
        body: {
...
export const updatePOHandler = async (updatePOvalues) => {
  ...
  if (targetExists) {
    ...
    // Database
    const { data: updatedPO } = await request(
      {
        url: process.env.API.PO,
        method: 'PATCH',
        params: {
  ...
  if (targetParentExists) { // PO-refId found ?
    ...
    if (targetExists) {
      ...
      const res = await httpParams(
        `${process.env.API.PO_ITEM}?poUUID=${targetParentUUID}&itemUUID=${targetUUID}`,
        'DELETE'
      );
      // Client Side
  ...
  if (targetParentExists) {
    ...
    if (isTargetSetValid) {
      ...
      if (isUnique) { // No Duplicate PO Item present already
        ...
        // Database
        const res = await httpParams(
          `${process.env.API.PO_ITEM}?poUUID=${targetParentUUID}`,
          'POST',
          {
...
export const updatePOitemSpecHandler = async ([activePOid, activeItemIndex, specFormData]) => {
  ...
  if (targetParent2Exists) { // PO-refId found ?
    ...
    if (targetParent) { // PO-itemId found ?
      ...
      // Database
      const res = await httpParams(
        `${process.env.API.PO_ITEM_SPEC}?poUUID=${targetParent2UUID}&itemUUID=${targetParentUUID}`,
        'PATCH',
        {
