...
export const addTxnHandler = async (txnData) => {
  ...
  if (isUnique) {
    ...
    console.log("created Transaction @ app state: ", createdTransaction);
    // 2. Client Side
...
export const addTxnListHandler = async (txnListData) => {
  ...
  // 2. Client Side
  createdTransactionList.forEach(
    transaction => {
...
export function mapPOtoTransaction(data = {}, poItemIdx = Math.round(Math.random() * 9), totalPoItems = 999, refType, refId, supplier) {
  ...
  const test = `${refType}#${refId}_${poItemIdx}/${totalPoItems}_${epochTimeOfTxn}`;
  return {
    date: epochTimeOfTxn,
    ...
    */
    ...
    tid: test, // auto-generated transaction id
