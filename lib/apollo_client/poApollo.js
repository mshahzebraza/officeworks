...
import { addTxnHandler, addTxnListHandler, mapPOtoTransaction } from './transactionApollo';
...
export const addPOHandler = async (newPOvalues) => {
  ...
  if (isUnique) {
    ...
    const { poData: createdPO } = await res.json();
    // Client Side
...
export const updatePOHandler = async (updatePOvalues) => {
  ...
  if (updatePOvalues.status === 'Closed') {
    ...
    // updatePOvalues.items.forEach((poItem, idx, items) => addTxnHandler(
    //   mapPOtoTransaction(
    //     poItem,
    //     idx,
    //     items.length,
    //     updatePOvalues?.refType,
    //     updatePOvalues?.refId,
    //     updatePOvalues?.supplier
    //   ))
    // )

    // convert PO items to transactions
    const poTxnList = updatePOvalues.items.map((poItem, idx, items) => mapPOtoTransaction(
      poItem,
      idx,
      items.length,
      updatePOvalues?.refType,
    ...
    addTxnListHandler(poTxnList)
    // poTxnList.forEach(txn => {
    //   setTimeout(() => addTxnHandler(txn), 100)
    //   // addTxnHandler(txn)
    // })
  }
