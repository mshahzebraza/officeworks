...
import mwoApollo from './mwoApollo';
...
// Target: PO/POItem
export const deletePOmoduleHandler = async ([activeSourceID, itemDeleteId]) => {
...
export const updatePOmoduleHandler = async ([activeSourceID, itemData]) => {
...
// Target: MWO/MWOItem
export const deleteMWOmoduleHandler = async ([activeSourceID, itemDeleteId]) => {
     ...
     const activeSourceIndex = mwoStateList.findIndex(mwo => mwo.mwoId === activeSourceID)
     const activeModuleIndex = moduleStateList.findIndex(module => module.id === itemDeleteId)
     if (activeSourceIndex === -1 || activeModuleIndex === -1) {
          console.error('Error: deleteMWOitemHandler: activeSourceIndex or activeModuleIndex is -1');
          return null;
     }
     const activeSource = mwoStateList[activeSourceIndex];
     const activeModule = moduleStateList[activeModuleIndex];
     const activeSourceUUID = activeSource._id
     const activeModuleUUID = activeModule._id
     ...
     const { success, data, error, message } = await request(
          {
               ...
               method: 'DELETE',
               params: {
                    mwoUUID: activeSourceUUID, // mwo.mwoId
                    moduleUUID: activeModuleUUID // item.id
               }
          }
     )
     ...
     // 2. Client update
     { // Update the source state (unlink - already done in the server, so just replace the data in the state with server's)
          mwoStateList[activeSourceIndex] = moduleSource
     }
     { // Update the module state (Delete or unlink)
          // ? Exclusive means that "it contains other po refs beside the current po ref in the linkedPOs"
          // Exclusive: delete the module altogether
          ...
          // Stop execution if the module to be removed is not linked to any other modules
          if (!activeModule.linkedMWOs.includes(moduleSource._id)) {
               console.error('Not Possible: Module target is not linked to current PO. Returning null');
               return null;
          }

          const isLastInstance =
               activeModule.linkedMWOs.length === 1
               && activeModule.linkedMWOs[0] === activeSourceUUID
               && activeModule.linkedPOs.length === 0;
...
export const addMWOmoduleHandler = async ([activeSourceID, itemData]) => {
     // TODO: update the fetch property of the states
     ...
     if (existingModuleIndex !== -1) {
          const existingModule = moduleStateList[existingModuleIndex]
          console.log('hitting existing module');
          const existsInActiveMWO = existingModule.linkedMWOs?.includes(activeSourceUUID)

          if (!!existsInActiveMWO) {
               alert('This module already exists in the active PO. Cannot add duplicate in the same PO.')
               return null
          }
          // if duplicate module found, then ask user to confirm if they want to proceed
...
export const updateMWOmoduleHandler = async ([activeSourceID, itemData]) => {
...
// Target: MWO-Item-Specs
export const updateModuleSpecHandler = async ([specFormData]) => {
     const moduleState = moduleApollo()
     const moduleStateList = [...moduleState.list]
...
export const addModuleHandler = async (itemData) => {
     // Ensure that the module does not already exist in the module list
     // add another entry to the module list
     console.log('addModuleHandler Data: ', itemData);
}
export const updateModuleHandler = async (itemData) => {

     console.log('updateModuleHandler Data: ', itemData);
}
export const deleteModuleHandler = async (moduleUUID) => {
     console.log('deleteModuleHandler UUID: ', moduleUUID);
